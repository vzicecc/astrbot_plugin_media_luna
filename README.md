...
插件页面通过 AstrBot 官方插件页面机制挂载在主 Dashboard（端口 6185）上，**不需要额外端口**：

- 访问方式：打开 AstrBot Dashboard → 插件管理 → 找到 media_luna → 打开其「页面/dashboard」；或在侧边栏点击插件页面入口。
- 直接地址：`http://<主机>:6185/api/plug/media_luna/page/dashboard`（需先登录 Dashboard）。
